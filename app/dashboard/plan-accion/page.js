...
export default async function PlanAccionPage() {
  ...
  const [{ data: profile }, { data: planes }, { data: incidentes }, { data: profiles }] = await Promise.all([
    supabase.from("profiles").select("role").eq("id", user.id).single(),
    supabase.from("plan_accion").select("*").order("fecha_limite", { ascending: true, nullsFirst: false }),
    supabase.from("incidentes").select("id, fecha, descripcion").order("fecha", { ascending: false }),
  ...
  return (
    <div className="page-body">
      ...
      {canEdit && <NewPlanAccionForm incidentes={incidentes || []} profiles={profiles || []} />}
    </div>
  );
}
