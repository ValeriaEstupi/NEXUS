...
import { updateIncidente, deleteIncidente } from "@/app/lib/actions/incidentes";
import {
  formatFechaHora,
...
export default function IncidenteRow({ incidente, empresaId, vehiculos, conductores, canEdit, canDelete }) {
  ...
  function handleSave(formData) {
    ...
    startTransition(async () => {
      const res = await updateIncidente(incidente.id, data, empresaId);
  ...
  async function handleDelete() {
    ...
    const res = await deleteIncidente(incidente.id, empresaId);
    if (res?.error) setError(res.error);
  }
