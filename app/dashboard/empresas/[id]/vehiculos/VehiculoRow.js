...
import { updateVehiculo, deleteVehiculo } from "@/app/lib/actions/vehiculos";
import { estadoVencimiento, formatFecha, VEHICULO_ESTADO_LABEL } from "@/app/lib/helpers";

export default function VehiculoRow({ vehiculo, empresaId, canEdit, canDelete }) {
  ...
  function handleSave(formData) {
    ...
    startTransition(async () => {
      const res = await updateVehiculo(vehiculo.id, data, empresaId);
  ...
  async function handleDelete() {
    ...
    const res = await deleteVehiculo(vehiculo.id, empresaId);
    if (res?.error) setError(res.error);
  }
