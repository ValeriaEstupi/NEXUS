...
import { addRequisitoIso } from "@/app/lib/actions/cumplimiento";

export default function AddRequisitoIsoForm({ empresaId, normaId, fases }) {
  ...
  function handleSubmit(formData) {
    ...
    startTransition(async () => {
      const res = await addRequisitoIso({
        empresaId,
        normaId,
  ...
  if (!open) {
    return (
      <button type="button" className="secondary" onClick={() => setOpen(true)}>
        + Agregar requisito a esta norma
      </button>
    );
  }
  ...
  return (
    <form action={handleSubmit} className="form-grid" style={{ marginTop: 10 }}>
      ...
      <div style={{ maxWidth: 100 }}>
        ...
        <input name="codigo" placeholder="8.1" />
      </div>
