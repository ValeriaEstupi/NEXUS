...
import {
  ...
  deleteCapacitacion,
  toggleAsistente,
} from "@/app/lib/actions/capacitaciones";
import { formatFecha, CAPACITACION_TIPO_LABEL } from "@/app/lib/helpers";

export default function CapacitacionRow({ capacitacion, empresaId, conductores, canEdit, canDelete }) {
  ...
  function handleSave(formData) {
    ...
    startTransition(async () => {
      const res = await updateCapacitacion(capacitacion.id, data, empresaId);
  ...
  async function handleDelete() {
    ...
    const res = await deleteCapacitacion(capacitacion.id, empresaId);
    if (res?.error) setError(res.error);
  }

  async function handleToggleAsistente(conductorId, checked) {
    setError(null);
    const res = await toggleAsistente(capacitacion.id, conductorId, checked, empresaId);
    if (res?.error) setError(res.error);
  }
