...
import { createPilar, updatePilar, deletePilar } from "../../lib/actions/pilares";

export default function PilaresManager({ pilares, canDelete }) {
        ...
        <>
          ...
          <div className="data-table-wrap">
            <table className="data-table">
              ...
              <tbody>
                {pilares.map((p) => (
                  <PilarRow key={p.id} pilar={p} canDelete={canDelete} setError={setError} />
                ))}
              </tbody>
            </table>
          </div>
...
function PilarRow({ pilar, canDelete, setError }) {
  ...
  function handleSave(formData) {
    setError(null);
    startTransition(async () => {
      const res = await updatePilar(pilar.id, {
        nombre: formData.get("nombre"),
        descripcion: formData.get("descripcion"),
  ...
  async function handleToggleActivo() {
    setError(null);
    const res = await updatePilar(pilar.id, { activo: !pilar.activo });
    if (res?.error) setError(res.error);
  }

  async function handleDelete() {
    if (!confirm(`¿Borrar el pilar "${pilar.nombre}"?`)) return;
    const res = await deletePilar(pilar.id);
    if (res?.error) setError(res.error);
  }
