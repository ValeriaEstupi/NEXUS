...
import { updateConductor, deleteConductor } from "../../lib/actions/conductores";
import { estadoVencimiento, formatFecha, CONDUCTOR_ESTADO_LABEL } from "../../lib/helpers";

export default function ConductorRow({ conductor, vehiculos, canEdit, canDelete }) {
  ...
  function handleSave(formData) {
    ...
    startTransition(async () => {
      const res = await updateConductor(conductor.id, data);
  ...
  async function handleDelete() {
    ...
    const res = await deleteConductor(conductor.id);
    if (res?.error) setError(res.error);
  }
